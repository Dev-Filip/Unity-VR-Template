...
public class ArmSwinger : MonoBehaviour {
	...
	//private GameObject leftControllerTrackedObj;
	//private GameObject rightControllerTrackedObj;
	...
	void FixedUpdate() {
		...
		if (armSwingNavigation &&
			...
			movementDirection = variableArmSwingMotion();
	...
	void getControllerButtons() {
		...
		if (leftController.inputDevice.TryGetFeatureValue(CommonUsages.gripButton, out bool gripButtonleft))
				leftButtonPressed = gripButtonleft;

		if (rightController.inputDevice.TryGetFeatureValue(CommonUsages.gripButton, out bool gripButtonRight))
			rightButtonPressed = gripButtonRight;
